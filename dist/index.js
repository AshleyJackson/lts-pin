...
async function getPreviousVersion(packageName) {
    try {
        // Fetch package metadata from npm registry
        const response = await axios.get(`https://registry.npmjs.org/${packageName}`);
        const versions = Object.keys(response.data.versions)
            .filter((v) => semver.valid(v) && !semver.prerelease(v)) // Exclude pre-releases
...
async function updateToPreviousVersions(targetDir = process.cwd()) {
    try {
        ...
        for (const pkg of allPackages) {
            const version = await getPreviousVersion(pkg);
            if (version) {
        ...
        console.error(`Error updating package.json in ${targetDir}:`, error.message);
        process.exit(1);
    }
}
// CLI entry point
